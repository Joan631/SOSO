...
class ButtonSettingsScreen(Screen):
    def __init__(self, **kwargs):
        ...
        shake_box.bind(minimum_height=shake_box.setter('height'))
        ...
        self.shake_settings_box.bind(minimum_height=self.shake_settings_box.setter('height'))
        self.shake_settings_box.add_widget(Label(text="Shake Sensitivity"))
        self.shake_slider = Slider(min=1, max=10, value=BUTTON_SETTINGS["shake_sensitivity"], step=1, size_hint_y=None, height=50)
        ...
        voice_box.bind(minimum_height=voice_box.setter('height'))
        ...
        self.voice_settings_box.bind(minimum_height=self.voice_settings_box.setter('height'))
        self.voice_settings_box.add_widget(Label(text="Activation Phrase"))
        self.voice_input = TextInput(text=BUTTON_SETTINGS["voice_phrase"], multiline=False, size_hint_y=None, height=45)
        self.voice_settings_box.add_widget(self.voice_input)
        self.voice_settings_box.add_widget(Label(text="Voice Sensitivity"))
        self.voice_slider = Slider(min=1, max=10, value=BUTTON_SETTINGS["voice_sensitivity"], step=1, size_hint_y=None, height=50)
        ...
        tap_box.bind(minimum_height=tap_box.setter('height'))
        ...
        self.tap_count_input = TextInput(text=str(BUTTON_SETTINGS["tap_count"]), multiline=False, size_hint_y=None, height=45)
        ...
        customize_box.bind(minimum_height=customize_box.setter('height'))
        ...
        self.size_input = TextInput(text=str(BUTTON_SETTINGS["button_size"]), multiline=False, size_hint_y=None, height=45)
        ...
        security_box.bind(minimum_height=security_box.setter('height'))
        ...
        # Countdown seconds (hidden if not enabled)
        ...
        self.countdown_input = TextInput(text=str(BUTTON_SETTINGS["countdown_seconds"]), multiline=False, size_hint_y=None, height=45)
    ...
    def save_settings(self, instance):
        BUTTON_SETTINGS["shake_enabled"] = self.shake_chk.active
        BUTTON_SETTINGS["shake_sensitivity"] = int(self.shake_slider.value)
        BUTTON_SETTINGS["voice_enabled"] = self.voice_chk.active
        BUTTON_SETTINGS["voice_phrase"] = self.voice_input.text
        BUTTON_SETTINGS["voice_sensitivity"] = int(self.voice_slider.value)
        BUTTON_SETTINGS["tap_enabled"] = self.tap_chk.active
        ...
        BUTTON_SETTINGS["floating_enabled"] = self.floating_chk.active
        try:
        ...
        BUTTON_SETTINGS["countdown_enabled"] = self.countdown_chk.active
        try:
            BUTTON_SETTINGS["countdown_seconds"] = max(1, int(self.countdown_input.text))
        except ValueError:
            BUTTON_SETTINGS["countdown_seconds"] = 5

        # Update app-level settings for main.py
        app = App.get_running_app()
        app.button_settings = BUTTON_SETTINGS

        Popup(title="Saved", content=Label(text="Button settings saved!"),
